...
public:
  ...
  void start(Node** _node, const VarNote::Currency* currency,  INodeCallback* _callback, Logging::LoggerManager* _loggerManager,
    const VarNote::CoreConfig& _coreConfig, const VarNote::NetNodeConfig& _netNodeConfig) {
    ...
    try {
      ...
      Q_EMIT nodeInitFailedSignal(VarNote::error::INTERNAL_WALLET_ERROR);
      QCoreApplication::processEvents();
      return;
    }
...
NodeAdapter::NodeAdapter() : QObject(), m_node(nullptr), m_nodeInitializerThread(), m_nodeInitializer(new InProcessNodeInitializer) {
  ...
  qRegisterMetaType<VarNote::CoreConfig>("VarNote::CoreConfig");
  qRegisterMetaType<VarNote::NetNodeConfig>("VarNote::NetNodeConfig");
...
VarNote::IWalletLegacy* NodeAdapter::createWallet() const {
  Q_CHECK_PTR(m_node);
...
bool NodeAdapter::init() {
  Q_ASSERT(m_node == nullptr);
  QUrl localNodeUrl = QUrl::fromUserInput(QString("127.0.0.1:%1").arg(VarNote::RPC_DEFAULT_PORT));
...
bool NodeAdapter::initInProcessNode() {
  ...
  VarNote::CoreConfig coreConfig = makeCoreConfig();
  VarNote::NetNodeConfig netNodeConfig = makeNetNodeConfig();
...
VarNote::CoreConfig NodeAdapter::makeCoreConfig() const {
  VarNote::CoreConfig config;
...
VarNote::NetNodeConfig NodeAdapter::makeNetNodeConfig() const {
  VarNote::NetNodeConfig config;
  boost::program_options::variables_map options;
