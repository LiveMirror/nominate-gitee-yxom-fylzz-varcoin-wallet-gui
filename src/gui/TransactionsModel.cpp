...
TransactionsModel::TransactionsModel() : QAbstractItemModel() {
  ...
  connect(&WalletAdapter::instance(), &WalletAdapter::walletTransactionCreatedSignal, this,
    static_cast<void(TransactionsModel::*)(VarNote::TransactionId)>(&TransactionsModel::appendTransaction), Qt::QueuedConnection);
...
QVariant TransactionsModel::data(const QModelIndex& _index, int _role) const {
  ...
  VarNote::TransactionId transactionId = m_transfers.value(_index.row()).first;
  VarNote::TransferId transferId = m_transfers.value(_index.row()).second;

  if(!WalletAdapter::instance().getTransaction(transactionId, transaction) ||
    (m_transfers.value(_index.row()).second != VarNote::WALLET_LEGACY_INVALID_TRANSFER_ID &&
...
QVariant TransactionsModel::getUserRole(const QModelIndex& _index, int _role, VarNote::TransactionId _transactionId,
  VarNote::WalletLegacyTransaction& _transaction, VarNote::TransferId _transferId, VarNote::WalletLegacyTransfer& _transfer) const {
  switch(_role) {
  case ROLE_DATE:
  ...
  case ROLE_AMOUNT:
    return static_cast<qint64>(_transferId == VarNote::WALLET_LEGACY_INVALID_TRANSFER_ID ? _transaction.totalAmount : -_transfer.amount);
  ...
  case ROLE_NUMBER_OF_CONFIRMATIONS:
    return (_transaction.blockHeight == VarNote::WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT ? 0 :
...
void TransactionsModel::reloadWalletTransactions() {
  ...
  quint32 row_count = 0;
  for (VarNote::TransactionId transactionId = 0; transactionId < WalletAdapter::instance().getTransactionCount(); ++transactionId) {
...
void TransactionsModel::appendTransaction(VarNote::TransactionId _transactionId, quint32& _insertedRowCount) {
  VarNote::WalletLegacyTransaction transaction;
  ...
  if (transaction.transferCount) {
    ...
    for (VarNote::TransferId transfer_id = transaction.firstTransferId;
    ...
    m_transfers.append(TransactionTransferId(_transactionId, VarNote::WALLET_LEGACY_INVALID_TRANSFER_ID));
    m_transactionRow[_transactionId] = qMakePair(m_transfers.size() - 1, 1);
    ++_insertedRowCount;
  }
}

void TransactionsModel::appendTransaction(VarNote::TransactionId _transactionId) {
...
void TransactionsModel::updateWalletTransaction(VarNote::TransactionId _id) {
